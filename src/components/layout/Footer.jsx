      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <ul className="space-y-1 text-xs">
              {NAVIGATION_LINKS.filter(l => !l.dropdown).map((link) => (
                <li key={link.name}>
                  <Link 
                    to={link.path} 
                    className="hover:text-gold-400 transition-colors flex items-center gap-1.5 text-espresso-200 py-3 min-h-[48px]"
                  >
              ...
              <li>
                <Link to="/products" className="hover:text-gold-400 transition-colors flex items-center gap-1.5 text-espresso-200 font-semibold py-3 min-h-[48px]">
                  <span className="text-gold-500">›</span>
            ...
            <ul className="space-y-1 text-xs">
              {CATEGORIES_LIST.map((cat) => (
                <li key={cat.id}>
                  <Link 
                    to={cat.path} 
                    className="hover:text-gold-400 transition-colors flex items-center justify-between text-espresso-200 py-3 min-h-[48px]"
                  >
                    <span>{cat.name}</span>
                    <span className="text-[10px] text-espresso-400 bg-espresso-800 px-2 py-1 rounded font-bold">
                      {cat.count}
                    </span>
                  </Link>
                </li>
          ...
          <div>
            ...
            <address className="not-italic text-xs text-espresso-200 space-y-3 leading-relaxed">
              ...
              <p>
                ...
                <a href={`tel:${COMPANY_DETAILS.phone.replace(/\s+/g, '')}`} className="text-gold-400 hover:underline inline-flex items-center min-h-[48px] py-2">
                  {COMPANY_DETAILS.phone}
                </a>
              </p>
              <div>
                <strong className="text-cream-100 block mb-1">Department Desk Emails:</strong>
                <a href={`mailto:${COMPANY_DETAILS.emails.info}`} className="flex items-center text-espresso-300 hover:text-gold-400 min-h-[48px] py-1.5">
                  {COMPANY_DETAILS.emails.info}
                </a>
                <a href={`mailto:${COMPANY_DETAILS.emails.marketing}`} className="flex items-center text-espresso-300 hover:text-gold-400 min-h-[48px] py-1.5">
        ...
        <div className="pt-8 flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-espresso-400">
          ...
          <div className="flex items-center gap-4">
            <Link to="/contact" className="hover:text-gold-400 transition-colors inline-flex items-center justify-center px-2 py-3 min-h-[48px]">Privacy Policy</Link>
            <Link to="/contact" className="hover:text-gold-400 transition-colors inline-flex items-center justify-center px-2 py-3 min-h-[48px]">Terms of Business</Link>
            <a href="/sitemap.xml" target="_blank" rel="noreferrer" className="hover:text-gold-400 transition-colors inline-flex items-center justify-center px-2 py-3 min-h-[48px]">Sitemap</a>
          </div>
        </div>
      </div>
    </footer>
