...
export default function ScrollableFoodShowcase() {
  ...
  // Selected cases state per product ID
  ...
  return (
    <section id="b2b-food-showcase" className="py-20 bg-cream-100 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Section Header & Smooth Scroll Arrow Buttons */}
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
          <SectionHeader
            badge="Our Product Collection"
            title="Explore the ROSE Product Range"
            subtitle="From classic tea biscuits to rich butter cookies and crisp papad, discover our complete variety crafted with care."
            align="left"
          />

          {/* Smooth Scroll Navigation Arrows (Scrolls 4 Defined Cards at a Time) */}
          <div className="flex items-center gap-2.5 self-start md:self-end">
            <button
              onClick={() => scroll('left')}
              aria-label="Scroll left"
              className="p-3.5 rounded-xl bg-cream-50 border border-espresso-950/15 text-espresso-800 hover:bg-maroon-900 hover:text-gold-400 transition-all shadow-sm active:scale-95"
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all border ${
                activeCategory === cat.id
                  ...
                  : 'bg-cream-50 text-espresso-800 border-espresso-950/10 hover:bg-cream-200'
              }`}
            >
              {cat.name}
            </button>
        ...
        {/* Defined 4-Card Scrollable Track (Mathematically 4 Cards Fit 100% With Zero Cutoff) */}
              ...
              {displayProducts.map((product) => {
                ...
                return (
                  <motion.div
                    ...
                    className="snap-start flex-shrink-0 w-full sm:w-[calc((100%-1.5rem)/2)] lg:w-[calc((100%-4.5rem)/4)] bg-cream-50 border border-espresso-950/10 rounded-2xl p-4 shadow-warm hover:shadow-warm-hover transition-all flex flex-col justify-between"
                  >
                    {/* Top Image Box with Food Backdrop */}
                    <div className="relative mb-3 rounded-xl overflow-hidden bg-cream-200 aspect-[4/3]">
                      {product.badge && (
                        <span className="absolute top-2.5 left-2.5 z-20 px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wider bg-maroon-900 text-gold-400 border border-gold-500/30 shadow-xs">
                          {product.badge}
                        </span>
                      )}
                    ...
                    <div className="flex-1 flex flex-col justify-between">
                      ...
                      {/* Visual "1 Case = X Units" Indicator */}
                      ...
                      <div className="mt-4 pt-3 border-t border-cream-200 space-y-2">
                        ...
                        <RoseCTAButton
                          variant="primary"
                          size="compact"
                          label={inCartItem ? `✓ ${inCartItem.cases} Cases in Quote` : `Add ${casesCount} Cases to Quote`}
